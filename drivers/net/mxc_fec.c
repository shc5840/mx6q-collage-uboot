...
#define PHY_MARVEL_OUID_GET(valh, vall) \
				((vall << 6) |(valh>>10))
...
#define phy_is_ar8033()	(((fec_phy_ouid & PHY_OUID_MASK) == PHY_AR8033_OUID) \
			? 1 : 0)
#define phy_is_88e138() ((fec_phy_ouid  == 0x5043)  ? 1 : 0)
...
static void mxc_get_phy_ouid(char *devname, int phy_addr)
{
	...
	fec_phy_ouid = PHY_MARVEL_OUID_GET(value2, value1);
	if (phy_is_88e138())
		return;
			      ...
			      int dup_spd)
{
	...
	if (ret) {
		...
		} else if (phy_is_88e138()) { /* for Marvell phy */
			printf("PHY device:phy_is_88e138\n");
			ret = __fec_mii_read(fecp, addr, PHY_1000BTSR, &val);
			if (ret)
				dup_spd = _100BASET | (FULL << 16);
			else {
				if (val & (3 <<10)){
